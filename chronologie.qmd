...
Pour une étude clinique correcte il faut prévoir environ 5 ans. Mais rassurez-vous pour une thèse, sur un travail simple, on peut aller beaucoup plus vite. Il faut tout de même compter un an au minimum. 
...
C'est la partie la plus importante, la plus technique, la plus rébarbative de votre travail. Vous allez devoir décrire votre population, vos critères d'inclusion et d'exclusion, vos méthodes statistiques, vos tests, vos hypothèses, vos variables, etc.
...
Le seul calcul qui doit être fait à ce stade est le nombre de cas nécessaires. On se base sur la bibliographie & des méthodes statistiques variées qui vont d'un simple calcul de puissance à des méthodes de bootstrap par ex. 
...
Toutes ces données il va falloir les stocker. Deux solutions simples existent (pour des études monocentriques) : 

- Un formulaire en ligne type [Google Forms®](https://www.google.fr/intl/fr/forms/about/) ou [LymeSurvey®](https://www.limesurvey.org/fr) par ex.). C'est simple, rapide, gratuit mais attention à la sécurité : Google Form® est conforme au RGDP mais pas à la loi HIPAA donc ne devrait pas être utilisé pour des données de santé. Si les données sont complètement anonymisés ça peut passer (Un homme né le 12/03/47 pesant 76 Kg & hospitalisé du 12 au 17/03/23 en Médecine B ce n'est pas anonyme, on peut retrouver de qui il s'agit).
...
Pour un tableur je peux soit le créer complètement (mauvaise solution) soit simplement vérifier voire améliorer votre travail. 
:::
...
- De base j'utilise des échelles de couleurs compatibles avec les troubles de la vision des couleurs. De toute façon vous pouvez me demander de changer la couleur ou autre de n'importe quel élément. C'est votre étude, vos choix.
:::
