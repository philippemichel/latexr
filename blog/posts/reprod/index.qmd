...
- "Sympa ta présentation des résultats, on pourrait en faire un surveillance mensuelle" (de la part du chef, impossible de dire non)
...
Facile dés qu'on utilise Rmarkdown ou [Quarto](https://quarto.org/). c'est à dire un fichier en Markdown qui contient votre texte mais surtout du code découpé au morceaux appelés "chunks". On compile & on sort un beau rapport en pdf, html ou autre. On ne fait pas plus simple ! Et quand on reprend le travail tout est là, dans le même fichier. On peut le relancer avec une base de données qui a grossi (tant qu'elle garde la même structure).
...
Comme ça quand vous travailler à fignoler un graphique par exemple vous n'avez à relancer à chaque fois que ce chunk court donc simple & lisible. Ce qui fait pour un résultat simple un chunck de calculs pour la mise en forme des données etc., un chunck pour le tableau & un dernier pour le graphique. 
...
N'oubliez pas, outre les sauvegardes, d'utiliser un gestionnaire de version (Git ou autre) & d'avoir une gestion des fichiers un peu réfléchie, ne pas avoir plusieurs versions du même document dans le même dossier par ex.
